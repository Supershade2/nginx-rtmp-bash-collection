...
if [ "$selection" = "1" ]; then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[0] | .digitalocean' )"
elif [ "$selection" = "2" ]
then
echo -e "\n" 
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[0] | .url' )"
fi
done
fi
elif [ "$answer" = "2" ]
...
if [ "$answer" = "y" ]; then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[1] | .url' )"
fi
elif [ "$answer" = "3" ]
then
...
6.$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[5] | .name' )\n
7.$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[6] | .name' )\n
8.$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[7] | .name' )\n";
sentinal="";
while [ "$sentinal" != "q" ]
do
echo -e "$prompt"
...
then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[2] | .obs' )"
elif [ "$selection" = "3" ]
then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[2] | .xsplit' )"
fi
done
fi
elif [ "$answer" = "4" ]
...
if [ "$answer" = "y" ]; then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[5] | .url' )"
fi
elif [ "$answer" = "7" ]
then
...
if [ "$selection" = "1" ]; then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[6] | .rtmp' )"
elif [ "$selection" = "2" ]
then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[6] | .nginx' )"
fi
done
fi
elif [ "$answer" = "8" ]
then
echo -e "$(cat /usr/local/sbin/documentation/doc.json | jq -r '.[7] | .brief' )\nSee more y/n:"
read answer
if [ "$answer" = "y" ]; then
selection="";
while [ "$selection" != "3" ]
...
if [ "$selection" = "1" ]; then
echo -e "\n"
echo "$(cat /usr/local/sbin/documentation/doc.json | jq -r '.[7] | .mono' )"
elif [ "$selection" = "2" ]
then
echo -e "\n"
echo "$( cat /usr/local/sbin/documentation/doc.json | jq -r '.[7] | .template' )"
fi
done
fi
fi
